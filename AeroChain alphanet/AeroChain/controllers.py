...
def lookup():
    ...
    if form.accepted:
       ...
       #get most recent radix info
        ...
        url = "https://alphanet.radixdlt.com/v0/state/component"
        ...
        print(vals.find("nf_ids_hex"))
        print(vals[vals.find("nf_ids_hex"):])
        newVals = vals[vals.find("nf_ids_hex"):]
        print(newVals.find("]"))
        print(newVals[13:newVals.find("]")])
        newerVals = newVals[13:newVals.find("]")].replace('"', '')
        print(newerVals)
        valsArray = newerVals.split(",")
        print(valsArray)                            #array of all of the nfts
        print(len((valsArray)))
        
        #go through nfts and search
        for a in range(len(valsArray)):
            url = "https://alphanet.radixdlt.com/v0/state/non-fungible"
            ...
            data = """
            {
            "resource_address": "resource_tdx_a_1qpsc4wesnr24vfu9ehfh2qldynucycdey7l7kfjvdmjs0z90kl","""
           # "non_fungible_id_hex": "0a0500000000000000"
           # }
           # """
            data = data + '"non_fungible_id_hex": "'+valsArray[a]+'"}' 
            ...
            #print(resp.json())
            print(a)
            print(resp.json()["non_fungible"]["non_fungible_data"]["immutable_data"]["struct_data"]["data_json"]["fields"])
            print(len(resp.json()["non_fungible"]["non_fungible_data"]["immutable_data"]["struct_data"]["data_json"]["fields"]))

            nft = []
            for x in resp.json()["non_fungible"]["non_fungible_data"]["immutable_data"]["struct_data"]["data_json"]["fields"]:
                nft.append(x["value"])
                #append the 4 values
            if nft[0] == form.vars['First_Name'] and nft[1] == form.vars['Last_Name']:
                rows.append({'First_Name': nft[0], 'Last_Name': nft[1], 'Reason': nft[2]})
...
def register():
    ...
    if form.accepted:
        ...
        if counter is not 0:
            ...
            output = subprocess.getoutput(showNFTS)
            print(output)
            redirect("http://127.0.0.1:8080/crafty.html?FirstName="+form.vars['First_Name']+"&LastName="+form.vars['Last_Name']+"&BirthDate="+form.vars['Birth_Date']+"&Reason="+form.vars['Reason'])
            redirect(URL('accepted'))
        else:
            redirect(URL('not_accepted'))
    if form.errors:
